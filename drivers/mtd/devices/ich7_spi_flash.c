...
static const unsigned char sst25_opcode[] = {
	...
	0x04, 0x90, 0x9F,	/* wrdi      rdid      jedec_id     */
			       ...
			       const u8 *data, int length)
{
	ich7_spi_cmd_write(spi, sst25_opcode[sst25_prog], addr,
			   sst25_opcode[sst25_wren], length, data);
...
{
	struct ich7_spi_flash *s = platform_get_drvdata(p);
...
static int ich7_spi_mtd_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if ((addr & (mtd->erasesize - 1)) || (len & (mtd->erasesize - 1))) {
		printk( KERN_ERR "%s: Unaligned erase request\n",
		      drvname);
		return -EINVAL;
	}
			     ...
			     size_t * retlen, u_char * buf)
{
	...
	*retlen = 0;
	...
	if ((from + len) > mtd->size) {
		printk( KERN_ERR "%s: Attempt to read beyond end of "
		      "device\n", drvname);
		return -EINVAL;
	}
			      ...
			      size_t * retlen, const u_char * buf)
{
	...
	*retlen = 0;
	...
	if ((to + len) > mtd->size) {
		printk( KERN_ERR "%s: Attempt to write beyond end of "
		      "device\n", drvname);
		return -EINVAL;
	}
	...
	if (*retlen < len) {
		while ((len - *retlen) > 0) {
			iBufferSize = len - *retlen;
			if ( iBufferSize > MAX_WRITE_SIZE )
			{
				iBufferSize = MAX_WRITE_SIZE;
			}
			*retlen += ich7_spi_flash_prog(s->spi, to + *retlen,
						       buf + *retlen, iBufferSize);
		}
	}
...
static int ich7_spi_flash_resume(struct platform_device *dev)
{
	struct ich7_spi_flash *s = platform_get_drvdata(dev);
...
static int ich7_spi_flash_suspend(struct platform_device *dev, pm_message_t state)
{
	struct ich7_spi_flash *s = platform_get_drvdata(dev);
	struct mtd_info *mtd = &s->mtd;
	int err = 0;

	err = mtd->_suspend(mtd);

	return err;
}

static int __exit ich7_spi_flash_remove(struct platform_device *dev)
{
	struct ich7_spi_flash *s = platform_get_drvdata(dev);
...
static const struct flash_info __initdata flash_data[] = {
	...
	{"S25FL164", 	MFR_AMD, 0x16, 0x4017, 8 * 1024 * 1024, 64 * 1024, 
	 SST25VFXXXB_CMDS},
	{"AT25SF641", 	MFR_ADESTO, 0x16, 0x3217, 8 * 1024 * 1024, 64 * 1024, 
	 SST25VFXXXB_CMDS},
};
...
static struct pci_device_id ich7_pci_tbl[] = {
	...
	{PCI_DEVICE(PCI_VENDOR_ID_INTEL, PCI_DEVICE_ID_INTEL_QUARK_X1000_ILB)},
	{.vendor = 0,},
};

static int ich7_spi_flash_probe(struct platform_device *dev)
{
	struct ich7_spi_flash *s;	
	...
	struct resource ich7_spi_flash_res[2];

    s = devm_kzalloc(&dev->dev, sizeof(struct ich7_spi_flash), GFP_KERNEL);
    if (!s)
        return -ENOMEM;

    platform_set_drvdata(dev, s);

	for (pci_id = ich7_pci_tbl; pci_id->vendor; pci_id++) {
		s->pci = pci_get_device(pci_id->vendor, pci_id->device, NULL);
		if (s->pci) {
			u32 rcba;
	...
	err = -ENOMEM;
	s->spi = ioremap_nocache(ich7_spi_flash_res[0].start,
				 ich7_spi_flash_res[0].end - ich7_spi_flash_res[0].start + 1);
	if (!s->spi)
		goto out;

	/* initialize the SPI controller so we can probe the flash */
	err = ich7_spi_startup(dev);
	if (err)
		goto unmap_spi;
	...
	if (i == ARRAY_SIZE(flash_data)) {
		printk( "Flash component with mfr id 0x%02x "
			 "and device id 0x%02x is not supported.\n", mfr, id);
		err = -ENODEV;
		goto spi_shutdown;
	}

	dev_info(&dev->dev, "Found %s, size %dKB\n", f->name, f->size >> 10);
	...
	s->mtd.name = drvname;
	s->mtd._erase = ich7_spi_mtd_erase;
	s->mtd._read = ich7_spi_mtd_read;
	s->mtd._write = ich7_spi_mtd_write;
	s->mtd.writesize = 1;
	s->mtd._sync = NULL;
	s->mtd._lock = NULL;
	s->mtd._unlock = NULL;
	s->mtd._suspend = ich7_spi_mtd_suspend;
	s->mtd._resume = ich7_spi_mtd_resume;
	s->mtd.priv = dev;
...
static struct platform_driver ich7_spi_flash_driver = {
	.probe = ich7_spi_flash_probe,
	.remove = ich7_spi_flash_remove,
...
{
	platform_driver_unregister(&ich7_spi_flash_driver);
}
static int __init ich7_spi_flash_init(void)
{
	struct platform_device *ich7_device;
	int err = -ENODEV;

	err = platform_driver_register(&ich7_spi_flash_driver);

    ich7_device = platform_device_register_simple(drvname,
                                -1, NULL, 0);
    if (IS_ERR(ich7_device)) {
        pr_err("unable to register device\n");
		platform_driver_unregister(&ich7_spi_flash_driver);
        return PTR_ERR(ich7_device);
    }
